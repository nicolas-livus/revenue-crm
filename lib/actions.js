...
const refresh = () => ["/", "/database", "/cadencias", "/recuperacao"].forEach(p => revalidatePath(p));
...
export async function createProspect(fd) {
  ...
  const [p] = await sql`insert into prospects (code, strategy, name, company, title, linkedin, email, phone, has_whatsapp, owner, created_on)
...
export async function saveProspect(fd) {
  ...
  await sql`update prospects set strategy=${v(fd, "strategy")}, name=${v(fd, "name")}, company=${v(fd, "company")}, title=${v(fd, "title")}, linkedin=${v(fd, "linkedin")},
...
export async function importSheet(fd) {
  ...
  await sql.query(`
    ...
    select code, strategy, name, company, title, linkedin, email, phone, has_whatsapp, coalesce(stage,'Prospect'), coalesce(pipeline,'fora de pipe'),
    ...
    on conflict (code) do update set
      ...
      company=coalesce(excluded.company, prospects.company), title=coalesce(excluded.title, prospects.title), linkedin=coalesce(excluded.linkedin, prospects.linkedin),
...
export async function markInvalidWhatsapp(fd) {
  const id = Number(fd.get("id"));
  const stamp = new Date().toLocaleDateString("pt-BR", { timeZone: "America/Sao_Paulo" });
  await sql`update prospects set has_whatsapp = 'Não',
    notes = concat_ws(E'\n', notes, ${"WhatsApp inválido (marcado em " + stamp + ")."}::text), updated_at = now() where id = ${id}`;
  refresh();
}
